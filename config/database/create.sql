...
CREATE TABLE Capteur (
		idCapteur VARCHAR(32),
		nom VARCHAR(32),
    description text,
    frequency int4,
		type VARCHAR(32) NOT NULL REFERENCES TypeCapteur,
		PRIMARY KEY (idCapteur)
);

CREATE TABLE CapteurPhy (
		idCapteur VARCHAR(32) NOT NULL REFERENCES Capteur,
		timeStart time,
    timeStop time,
    boardPin VARCHAR (8),
    endPointIP VARCHAR (32),
    port int4,
    idBoard VARCHAR (32) REFERENCES Board,
		PRIMARY KEY (idCapteur)
);
...
CREATE TABLE CorrespondT (
		idCapteur VARCHAR(32) NOT NULL REFERENCES Capteur,
		name VARCHAR (32) NOT NULL REFERENCES Tag,
